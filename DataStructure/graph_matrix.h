...
// 顶点的动态操作
	...
	{
		for (int j = 0; j < n; j++)
			E[j].push_bakc(nullptr);
		n++;  //各顶点预留一条潜在的关联边
		E.push_back(std::vector<Edge<Te>*>(n, (Edge<Te>*) nullptr)); //创建新顶点对应的边向量
	...
	virtual Tv remove(int i)  //删除第i个顶点及其关联边（0 <= i < n）
	{
		...
		E.erase(E.begin() + i); n--;  //删除第i行
		Tv vBak = vertex(i); V.erase(V.begin()+i); //删除顶点i
		for (int j = 0; j < n; j++) //所有入边
			if (Edge<Te> * e = *(E[j].erase(E.begin()+i))) 
